...
const [salePriceModal, setShowSalePriceModal] = useState(false);
    const [salePrice, setSalePrice] = useState("");
    const [listorNot, setListorNot] = useState(false); //false->List, true->Unlist
  ...
  const ListorUnlistFromMarket = async (listorUnlist, ticketID) => {
    try {
      if (!listorUnlist) {

        console.log("Listing Process has begun.");
        //console.log(parseFloat(salePrice),typeof(parseFloat(salePrice))); // Output: "42.0"
        // console.log(ticketID,typeof(ticketID));
        console.log(parseInt(ticketID, 10), typeof parseInt(ticketID, 10));
        // try {
        //   const transactionId = await fcl.send([
        //     fcl.transaction(listForSaleTx),
        //     fcl.args([
        //       fcl.arg(0, t.UInt64),
        //       fcl.arg(parseFloat(salePrice), t.UFix64)
        //     ]),
        //     fcl.payer(fcl.authz),
        ...
        // } catch (error) {
        //   console.error('Error in Flow Blockchain transaction:', error);
        //   // Handle errors if necessary
        // }

        const salePrice2 = Math.ceil(parseFloat(salePrice))
        try {
          await contract.methods.ListTicketforSale(ticketID, salePrice2).send({ from: accounts[0] });
        } catch (error) {
          console.error('Error in Ethereum contract transaction:', error);
          // Handle errors if necessary
        }              
        ...
        console.log("Unlisting process has begun.");

        // try {
        //   const transactionId = await fcl.send([
        //     fcl.transaction(unlistFromSaleTx),
        //     fcl.args([
        //       fcl.arg(parseInt(ticketID,10), t.UInt64)
        //     ]),
        //     fcl.payer(fcl.authz),
        //     fcl.proposer(fcl.authz),
        //     fcl.authorizations([fcl.authz]),
        ...
        try {
          await contract.methods.unListTicketforSale(ticketID).send({ from: accounts[0] });
        } catch (error) {
          console.error('Error in Ethereum contract transaction:', error);
          // Handle errors if necessary
        }
      ...
      console.error('Error in ListorUnlistFromMarket:', error);
      // Handle errors if necessary
    }
  }
...
//   }, []);
  ...
  <a style={{ backgroundColor: 'transparent', color: 'inherit', textDecoration: 'none' }} href={`/dashboard/NFTicketsStore/${flowAddr}`}>
    ...
    {!isLoading && (<div className="user-dashboard">
      ...
      <h2 style={{textAlign:'center',marginBottom:"-4vh"}}>All Your Tickets at a Glance.</h2>
      </header>
      <table>
      <thead>
        <tr>
      ...
      <tbody>
        {tickets.map((ticket) => (
          <tr key={ticket.TicketNumber} style={{textAlign:'center'}}>
            ...
            <td>{ticket.isTicketAvailable && (<button onClick={()=>{!ticket.isTicketForSale?(setShowSalePriceModal(true)):(ListorUnlistFromMarket(ticket.isTicketForSale,ticket.TicketNumber))}}>{ticket.isTicketForSale?"Unlist from Sale":"Set Ticket for Sale"}</button>)}</td>
            <td>{parseInt(ticket.salePrice)?parseInt(ticket.salePrice):"0.0"}</td>
  ...
  <h4 style={{fontWeight:"700",color:'black'}}>Set Sale Price</h4>
        ...
        <Modal.Footer>
          ...
          <button onClick={()=>{setShowSalePriceModal(false)}} style={{backgroundColor:"#B04759",border:'solid 1px black',paddingLeft:'1vh',paddingRight:'1vh',textAlign:'center',color:"black"}}>
            Close
          </button>
          <button onClick={()=>{setShowSalePriceModal(false);ListorUnlistFromMarket(listorNot,ticket.TicketNumber)}} style={{color:"black",backgroundColor:"lightgreen",border:'solid 1px black',paddingLeft:'1vh',paddingRight:'1vh',textAlign:'center'}}>
