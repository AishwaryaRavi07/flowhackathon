...
//import setNFTUser from "../cadence/transactions/userSetUp";
...
function Login() {
    ...
    useEffect(()=>{
      ...
      const handleLogin = async (e) => {
        e.preventDefault();
        setIsLoading(true);  
        const obj = await contract.methods.GetPersonDetails(accountAddress).call();
        const personDetails = Object.values(obj);
        
        if(!(personDetails==null)){
            await FCL.authenticate(); // Assuming this is an asynchronous function, use await here      
            history.push(`/dashboard/${accountAddress}`);
              setIsAuthenticated(true);
              setIsLoading(false);
              setIsAuthenticated(true);
            }
        else{
          alert("Invalid EMail ID or Flow Account Address, please check again.");
        }
        ...
        <div style={{ alignSelf: 'center' }}>
          {!isAuthenticated && (
            <div style={{ width: '45vw' }}>
              <label>Enter Flow Account Addre:</label>
              <input type="text" required placeholder="Account Address" onChange={(e) => setAccountAddress(e.target.value)} />
